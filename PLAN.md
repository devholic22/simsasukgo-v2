...
- Backend: `Nest.js + TypeORM + Supabase(PostgreSQL/Auth)`
- 이유:
  - Nest.js 모듈 구조로 도메인/컨트롤러/서비스 계층을 명확히 분리하고 유지보수성을 높일 수 있음.
  - TypeORM으로 엔티티/리포지토리 기반 데이터 접근을 표준화할 수 있음.
  - Supabase PostgreSQL/Auth를 그대로 활용해 인증·데이터 인프라는 유지하면서 백엔드 애플리케이션 레이어는 Nest에서 통합 관리 가능.
...
- 환경변수/API Key 관리 원칙:
  - Google API Key는 서버(Nest API) 전용 비밀값으로 보관하고 클라이언트 번들에 포함하지 않음.
...
- 자동 동기화 구조:
  - 본체: `Nest Scheduler(@nestjs/schedule)` 기반 주기 호출
...
- 백엔드 패키지 구조는 `apps`(Controller/외부 인터페이스) + `libs`(CQRS 실행, 도메인, outbound 연동)로 구성한다.
